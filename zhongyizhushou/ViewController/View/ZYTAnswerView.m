...
@interface ZYTAnswerView()

@property(nonatomic,strong) UIView *bgView;

@end

@implementation ZYTAnswerView
...
- (void)creatAnswerButton{
    ...
    for (int i = 0; i<self.rightAnswer.length;i++) {
        ...
        [[button rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * x) {
            ...
            if ([obj isKindOfClass:[ZYTAnswerModel class]]) {
...
- (void)updateSubViews{
    ...
    for (int i=  0; i<self.Answer.count; i++) {
        ...
        else if ([obj isKindOfClass:[ZYTAnswerModel class]]){
            ZYTAnswerModel *model = (ZYTAnswerModel *)obj;
            [button setTitle:model.answer forState:UIControlStateNormal];
        }
    }
    BOOL selectAll =  [self.Answer bk_all:^BOOL(id obj) {
        return [obj isKindOfClass:[ZYTAnswerModel class]];
    }];
    if (selectAll) {
        NSArray *selectedAnswer = [self.Answer bk_map:^id(ZYTAnswerModel *obj) {
            return obj.answer;
        }];
